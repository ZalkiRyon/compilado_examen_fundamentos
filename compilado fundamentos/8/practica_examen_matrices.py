...
estado_asientos = np.full((7, 6), 'O')     # 'O' representa asiento disponible, 'X' representa asiento ocupado
...
def mostrar_asientos():
    ...
    for i in range(asientos.shape[0]):
        for j in range(asientos.shape[1]):
            if j == 3:
                print("\t", end="")
            print(f"{asientos[i, j]}{'X' if estado_asientos[i, j] == 'X' else 'O'}", end="\t")
        print("\n")
...
def anular_vuelo():
    ...
    try:
        ...
        fila, columna = divmod(num_asiento - 1, 6)
        if estado_asientos[fila, columna] == 'O':
